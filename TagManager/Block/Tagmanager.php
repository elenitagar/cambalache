...
class Panalysis_TagManager_Block_Tagmanager extends Mage_Core_Block_Template
{
    public function buildDataLayer()
    {
        ...
        if ($this->getPage() != 'Order Success') {
            ...
            $res=" ID ".$tmProduct['id']." SKU ".$tmProduct['sku']." ";
...
//                    $product = Mage::getModel('catalog/product')->loadByAttribute('sku',$sku);
                ...
                foreach ($productcollection as $product) {
                    ...
                    $product_array = array(
                        ...
                        'price' => number_format($item_price, 2),
                        'quantity' => (int)$item->getQtyOrdered(),
//  PRUEBAS                      'category' => $items[$product->getId()]['category'] PRUEBAS
                        'category' => $helper->getSkuCategory($sku)
                    );
                    if($brand = $tm->getBrand($product)) $product_array['brand'] = $brand;
            ...
            error_log("ICPDEV: Order ERROR ID: ".$order->getIncrementId()." Total ".$order->getGrandTotal());
            Mage::logException($e);
    }
    ...
    public function getCartProducts(){
        try
        {
            ...
            {
                $productId = $item->getProductId();
                /* vamos a ver si metemos los productos del variante
