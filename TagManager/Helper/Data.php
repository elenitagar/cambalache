...
//			$categoria1 .= $cat;
//			$categoria2 .= implode(",", $order['transactionProducts'][$k]);
 //           if($cat){
...
//			}
 //       }
        foreach($order['transactionProducts'] as $k => $p){
//			$cat = $this->getSkuCategory($p['id']);
			$catarray1 = Mage::getSingleton('core/session')->getGTMCartSkuCats();
			$cat = $catarray1[$k];
            if(!$order['transactionProducts'][$k]['category']){
                $order['transactionProducts'][$k]['category'] = $cat;
				$categoria1 .= implode(",", $cat);
            }
//            if($cat = $this->getSkuCategory($p['id'])){
//                $order['transactionProducts'][$k]['category'] = $cat;
//				$categoria1 .= implode(",", $cat);
//				}
			$categoria2 .= implode(",", $order['transactionProducts'][$k]);
...
//					'categoria2' => $categoria2
    ...
    public function buildCheckoutData($products){
        ...
        foreach($products as $k=>$v){
			$catarray = Mage::getSingleton('core/session')->getGTMCartSkuCats();
			$cat = $catarray[$k];
            if(!$products[$k]['category']){
        ...
        $data = array(
            ...
            'ecommerce' => array(
                ...
                'checkout' => array(
                    ...
                    'products' => $products
//					'categoria1' => implode ("-",Mage::getSingleton('core/session')->getGTMCartSkuCats())					
                )
            )
        );
